...
from networks.Framing import GoWithTheFlownet
...
if __name__ == '__main__':
    ...
    parser.add_argument('--crop_size', type=int, nargs='+', default = [270, 270], help="Spatial dimension to crop training samples for training")
    ...
    parser.add_argument('--validation_n_batches', type=int, default=2)
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if training:
            ...
            train_loader = load_data(data_path, args.effective_batch_size, train=True, shuffle=True, limit=0.01)
            frame_gen = GoWithTheFlownet()
            flownet = FlowNetC()
        ...
        # if exists(args.validation_dataset_root):
            ...
            validation_loader = load_data(data_path, args.effective_batch_size, train=False, shuffle=True, limit=0.1)
        ...
        # if exists(args.inference_dataset_root):
            ...
            inference_loader = load_data(data_path, args.effective_batch_size, train=False, shuffle=True, limit=0.1)
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
        ...
        if is_validate:
            ...
            progress = tqdm(tools.IteratorTimer(data_loader), ncols=120, total=np.minimum(len(data_loader), args.train_n_batches), smoothing=.9, leave=True, position=offset, desc=title)

        last_log_time = progress._time()
        for batch_idx, (data, target, idx) in enumerate(progress):
