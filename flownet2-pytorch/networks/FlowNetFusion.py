...
from utils.util import conv, predict_flow, deconv, crop_like, correlate
import math
import numpy as np
...
class FlowNetFusion(nn.Module):
    def __init__(self,args, batchNorm=True):
        ...
        self.conv0   = conv(self.batchNorm,  7,   64)
    ...
    def forward(self, x):
        ...
        out_deconv1 = crop_like(self.deconv1(out_conv2), out_conv1)
        
        concat1 = torch.cat((out_conv1,out_deconv1,flow2_up),1)
        out_interconv1 = self.inter_conv1(concat1)
        flow1       = self.predict_flow1(out_interconv1)
        flow1_up    = crop_like(self.upsampled_flow1_to_0(flow1), out_conv0)
        out_deconv0 = crop_like(self.deconv0(concat1), out_conv0)
