...
from networks.STNModule import SpatialTransformer
import torch.nn.functional as F
import torch.utils.checkpoint as cp
from utils.util import conv, conv_block, predict_flow, deconv, crop_like, correlate


class GoWithTheFlownet(nn.Module):
    def __init__(self, input_channels = 5, batchNorm=True):
        super(GoWithTheFlownet,self).__init__()
        self.decoder_l = Decoder()
...
class Decoder(nn.Module):
    def __init__(self, input_channels = 3, levels = 6, batchNorm=True):
        ...
        self.encoder = Encoder(input_channels=input_channels, batchNorm=batchNorm, enc_features=enc_features)

        self.decoders = []
        for i in range(len(enc_features)):
            if i < 5:
                self.decoders.append(DenseBlock(num_input_features=3 * enc_features[i]))
            else:
                self.decoders.append(DenseBlock(num_input_features=2 * enc_features[i]))
        ...
        self.trs = [SpatialTransformer(enc_features[i], level=i) for i in range(levels)]
