...
def main():
    ...
    # (Initialize logging)
    experiment = wandb.init(project='FlowNet', resume='allow', entity="oakstem")
    ...
    train_loader = load_data(data_path, args.batch_size, train=True, shuffle=True, limit=0.1)
    val_loader = load_data(data_path, args.batch_size, train=False, shuffle=True, limit=0.1)

    # create model
    if args.pretrained:
        network_data = torch.load(os.path.join(save_path, args.pretrained), map_location=device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if run_train:
            ...
            experiment = train(train_loader, model, optimizer, epoch, train_writer, experiment)
        # train_writer.add_scalar('mean EPE', train_EPE, epoch)

        scheduler.step()
        # evaluate on validation set
        with torch.no_grad():
            experiment = validate(val_loader, model, epoch, output_writers, experiment)
        ...
        }, is_best=False, save_path=save_path)



def train(train_loader, model, optimizer, epoch, train_writer, experiment):
    global n_iter, args
...
def validate(val_loader, model, epoch, output_writers, experiment):
    global args
    ...
    for i, (input, target, idx) in enumerate(val_loader):
        ...
        generate_img(target, output, max_val, args.div_flow)

        if i < 1:
            logging.info('Validation epe score: {}'.format(flow2_EPE.item()))
            experiment.log({
                'learning rate': args.lr,
                'validation loss': flow2_EPE.item(),
